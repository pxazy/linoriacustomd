...
function Library:FlatFill(Parent, ColorTop, ColorBottomMul, ZIndex)
    ...
    Library:AddToRegistry(grad, {
        Color = function()
            local c = Parent.Name == "Button" and Library.MainColor or Library.BackgroundColor
            return ColorSequence.new(c, Lighten(c, ColorBottomMul or 0.5))
        end
    })
...
function Funcs:AddButton(Text, Func)
    ...
    local Fill = Library:FlatFill(Outer, Library.MainColor, 0.55, 2)
    local Lbl = Library:CreateLabel({
        Size = UDim2.new(1, 0, 1, 0),
        Text = Text,
        TextXAlignment = Enum.TextXAlignment.Center,
...
function Funcs:AddToggle(Idx, Info)
    ...
    Library:AddToRegistry(Box, { BackgroundColor3 = 'BackgroundColor' })
...
function Funcs:AddSlider(Idx, Info)
    ...
    Library:AddToRegistry(Track, { BackgroundColor3 = 'BackgroundColor' })
...
function Funcs:AddDropdown(Idx, Info)
    ...
    Library:FlatFill(Box, Library.MainColor, 0.55, 2)
    ...
    Library:AddToRegistry(List, { BackgroundColor3 = 'BackgroundColor' })
...
function Funcs:AddColorPicker(Idx, Info)
    ...
    local pHeight = Library.IsMobileMode and 175 or 150

    local Popup = Library:Create('Frame', {
        BackgroundColor3 = Library.MainColor,
        BorderColor3 = Library.OutlineColor,
        BorderSizePixel = 1,
        Size = UDim2.new(0, 170, 0, pHeight),
        Visible = false,
        ZIndex = 60,
        Parent = ScreenGui,
    })
    ...
    local CopyBtn = Library:Create('TextButton', { BackgroundTransparency=1, Size=UDim2.new(1,0,0,22), Text="Copy", Font=Library.Font, TextSize=Library.FontSize, TextColor3=Library.FontColor, ZIndex=101, Parent=ContextMenu })
    local PasteBtn = Library:Create('TextButton', { BackgroundTransparency=1, Position=UDim2.new(0,0,0,22), Size=UDim2.new(1,0,0,22), Text="Paste", Font=Library.Font, TextSize=Library.FontSize, TextColor3=Library.FontColor, ZIndex=101, Parent=ContextMenu })
    Library:AddToRegistry(CopyBtn, { TextColor3 = 'FontColor' })
    Library:AddToRegistry(PasteBtn, { TextColor3 = 'FontColor' })

    CopyBtn.MouseButton1Click:Connect(function()
        Library.ColorClipboard = ColorPicker.Value
        ContextMenu.Visible = false
        Library.OpenedFrames[ContextMenu] = nil
    end)
    PasteBtn.MouseButton1Click:Connect(function()
        if Library.ColorClipboard then ColorPicker:SetValueRGB(Library.ColorClipboard) end
        ContextMenu.Visible = false
        Library.OpenedFrames[ContextMenu] = nil
    end)

    if Library.IsMobileMode then
        local MobCopyBtn = Library:Create('TextButton', {
            BackgroundColor3 = Library.BackgroundColor,
            BorderSizePixel = 0,
        ...
        local MobPasteBtn = Library:Create('TextButton', {
            BackgroundColor3 = Library.BackgroundColor,
            BorderSizePixel = 0,
            Position = UDim2.new(0, 88, 0, 150),
            Size = UDim2.new(0, 76, 0, 18),
            Text = "Paste",
            TextColor3 = Library.FontColor,
            Font = Library.Font,
        ...
        Library:AddToRegistry(MobCopyBtn, { BackgroundColor3 = 'BackgroundColor', TextColor3 = 'FontColor' })
        Library:AddToRegistry(MobPasteBtn, { BackgroundColor3 = 'BackgroundColor', TextColor3 = 'FontColor' })

        MobCopyBtn.MouseButton1Click:Connect(function()
            Library.ColorClipboard = ColorPicker.Value
        end)
        MobPasteBtn.MouseButton1Click:Connect(function()
            if Library.ColorClipboard then ColorPicker:SetValueRGB(Library.ColorClipboard) end
        end)
    end
    ...
    Swatch.InputBegan:Connect(function(Input)
        ...
        elseif Input.UserInputType == Enum.UserInputType.MouseButton2 then
            Library:CloseOpenedFrames()
            ContextMenu.Position = UDim2.fromOffset(InputService:GetMouseLocation().X, InputService:GetMouseLocation().Y)
            ContextMenu.Visible = true
            Library.OpenedFrames[ContextMenu] = true
        end
    end)

    Library:GiveSignal(InputService.InputBegan:Connect(function(Input)
        if Input.UserInputType ~= Enum.UserInputType.MouseButton1 and Input.UserInputType ~= Enum.UserInputType.Touch and Input.UserInputType ~= Enum.UserInputType.MouseButton2 then return end
        ...
        if Popup.Visible and not IsOver(Popup) and not IsOver(Swatch) then
            Popup.Visible = false
            Library.OpenedFrames[Popup] = nil
            SetActive(false)
        end
        
        if ContextMenu.Visible and not IsOver(ContextMenu) and not IsOver(Swatch) then
            ContextMenu.Visible = false
            Library.OpenedFrames[ContextMenu] = nil
        end
    end))
...
function Library:CreateWindow(Config)
    ...
    Library:FlatFill(Outer, Library.BackgroundColor, 0.7, 0)
    ...
    local TBarLine = Library:Create('Frame', { BackgroundColor3 = Library.AccentColor, BorderSizePixel = 0, Position = UDim2.new(0, 8, 1, -1), Size = UDim2.new(1, -16, 0, 1), Parent = TitleBar })
    Library:AddToRegistry(TBarLine, { BackgroundColor3 = 'AccentColor' })
    ...
    function Window:AddTab(Name)
        ...
        Library:AddToRegistry(TabBtn, { BackgroundColor3 = 'MainColor' })
        ...
        function Tab:AddGroupbox(Name, Side)
            ...
            local BoxOuter = Library:Create('Frame', { BackgroundColor3 = Library.MainColor, BorderSizePixel = 0, Size = UDim2.new(1, 0, 0, 40), Parent = Parent })
            Library:AddToRegistry(BoxOuter, { BackgroundColor3 = 'MainColor' })
            
            local gbLine = Library:Create('Frame', { BackgroundColor3 = Library.AccentColor, BorderSizePixel = 0, Size = UDim2.new(1, 0, 0, 2), Parent = BoxOuter })
            Library:AddToRegistry(gbLine, { BackgroundColor3 = 'AccentColor' })
...
if IsMobileMode then
    ...
    local function MakeBtn(text, y)
        ...
        local mobLine = Library:Create('Frame', { BackgroundColor3 = Library.AccentColor, BorderSizePixel = 0, Size = UDim2.new(1, 0, 0, 2), Parent = Outer })
        Library:AddToRegistry(mobLine, { BackgroundColor3 = 'AccentColor' })
...
local SaveManager = {} do
    ...
    SaveManager.Parser = {
        ...
        KeyPicker = {
            Save = function(idx, object) return { type = 'KeyPicker', idx = idx, key = object.Value } end,
            Load = function(idx, data) if Options[idx] then Options[idx]:SetValue({ data.key }) end end,
        },
    }
