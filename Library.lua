...
local function TryLoadCustomFont()
    local ok1, bytes = pcall(Base64Decode, ProggyCleanBase64)
    if not ok1 or not bytes then return nil end

    if not (writefile and getcustomasset) then return nil end

    local ok2 = pcall(writefile, "ProggyClean_LinoriaCustom.ttf", bytes)
    if not ok2 then return nil end

    local ok3, assetId = pcall(getcustomasset, "ProggyClean_LinoriaCustom.ttf")
    if not ok3 or not assetId then return nil end

    local ok4, fontFace = pcall(Font.new, assetId, Enum.FontWeight.Regular, Enum.FontStyle.Normal)
    if ok4 then return fontFace end

    return nil
end

local function ApplyCustomFont(Library)
    local CustomFontFace = TryLoadCustomFont()
    Library.CustomFontFace = CustomFontFace

    if not CustomFontFace then return end

    local OldCreateLabel = Library.CreateLabel
    function Library:CreateLabel(Properties, IsHud)
        local Inst = OldCreateLabel(Library, Properties, IsHud)
        pcall(function() Inst.FontFace = CustomFontFace end)
        return Inst
    end
end

local function AddTopFadeBar(Library, Window)
    ...
    Library:Create("UIGradient", {
        Transparency = NumberSequence.new({
            ...
            NumberSequenceKeypoint.new(0.92, 0.15),
...
local function BuildCompactMenu(Library, Parent, Categories, OnResize)
    local State = {
    ...
    Library:AddToRegistry(HeaderOuter, { BorderColor3 = "Black" })
    ...
    local HeaderLabel = Library:CreateLabel({
        ...
        TextSize = 14,
        Text = Categories[1] and Categories[1].Name or "--",
        ZIndex = 7,
        Parent = HeaderInner,
    })
    ...
    local ListOuter = Library:Create("Frame", {
        ...
        Size = UDim2.new(1, 0, 0, (#Categories * 18) + 2),
        Visible = false,
        ZIndex = 20,
        Parent = Outer,
    })
    ...
    local Grid = Library:Create("UIGridLayout", {
        ...
        CellSize = UDim2.new(0, 150, 0, 18),
        SortOrder = Enum.SortOrder.LayoutOrder,
        HorizontalAlignment = (#Categories == 1) and Enum.HorizontalAlignment.Center or Enum.HorizontalAlignment.Left,
        Parent = GridFrame,
    })

    local function Resize()
        Outer.Size = UDim2.new(1, 0, 0, 26 + GridFrame.Size.Y.Offset)
    ...
    local function BuildGrid(Category)
        for _, Child in next, GridFrame:GetChildren() do
            if not Child:IsA("UIGridLayout") then
                Child:Destroy()
            end
        end

        for _, OptionName in next, Category.Options do
            ...
            local Key = Category.Name .. "_" .. OptionName
            local Toggle = { Value = false, Callback = function(_) end }

            local function Display()
                BoxInner.BackgroundColor3 = Toggle.Value and Library.AccentColor or Library.MainColor
                BoxInner.BorderColor3 = Toggle.Value and Library.AccentColorDark or Library.OutlineColor
                Library.RegistryMap[BoxInner].Properties.BackgroundColor3 = Toggle.Value and "AccentColor" or "MainColor"
                Library.RegistryMap[BoxInner].Properties.BorderColor3 = Toggle.Value and "AccentColorDark" or "OutlineColor"
            end
            ...
            State.Toggles[Key] = Toggle
        end
    end

    BuildGrid(Categories[1])

    HeaderOuter.InputBegan:Connect(function(Input)
        if Input.UserInputType == Enum.UserInputType.MouseButton1 then
            ListOuter.Visible = not ListOuter.Visible
            ArrowLabel.Text = ListOuter.Visible and "^" or "v"
        end
    end)
    ...
    for _, Category in next, Categories do
        ...
        Library:OnHighlight(ItemLabel, ItemLabel,
...
function Extensions:Apply(Library, Window)
    ApplyCustomFont(Library)

    if Window then
        AddTopFadeBar(Library, Window)
    end

    function Library:AddCompactMenu(Parent, Categories, OnResize)
        return BuildCompactMenu(Library, Parent, Categories, OnResize)
    end
end
