...
function Funcs:AddInput(Idx, Info)
    ...
    Library:CreateLabel({ Size = UDim2.new(1, 0, 0, 14), Text = Info.Text or '', TextXAlignment = Enum.TextXAlignment.Left, Parent = Wrap })
    ...
    if Info.Finished then
        TextBox.FocusLost:Connect(function(enter) if enter then Commit() end end)
    else
        TextBox:GetPropertyChangedSignal('Text'):Connect(Commit)
    end
    ...
    function Input:OnChanged(f) Input.Changed = f; f(Input.Value) end
...
function Funcs:AddDropdown(Idx, Info)
    ...
    local MAX_DROPDOWN_ITEMS = 6

    local List = Library:Create('Frame', {
        BackgroundColor3 = Color3.fromRGB(20, 20, 20),
        BorderSizePixel = 0,
        Position = UDim2.new(0, 0, 1, 2),
        Size = UDim2.new(1, 0, 0, math.min(#Dropdown.Values, MAX_DROPDOWN_ITEMS) * 18),
        Visible = false,
        ZIndex = 50,
        Parent = Box,
    })
    ...
    local function BuildItems(Values)
        ...
        List.Size = UDim2.new(1, 0, 0, math.min(#Values, MAX_DROPDOWN_ITEMS) * 18)
    end

    BuildItems(Dropdown.Values)
...
function Library:CreateWindow(Config)
    ...
    if IsMobileMode and Library.MobileTitleLabel then
        Library.MobileTitleLabel.Text = Config.Title
    end
...
if IsMobileMode then
    ...
    local BTN_H, GAP = 24, 6
    local StartY = 96

    local TitleLabel = Library:Create('TextLabel', {
        BackgroundTransparency = 1,
        Position = UDim2.new(0, 10, 0, StartY - 18),
    ...
    Library.MobileTitleLabel = TitleLabel

    local function MakeBtn(text, y)
        local TextW = Library:GetTextBounds(text, Library.Font, Library.FontSize - 1)
        local Width = math.max(TextW + 20, 74)

        local Outer = Library:Create('Frame', {
            BackgroundColor3 = Color3.fromRGB(10, 10, 10),
            BackgroundTransparency = 0.1,
            Position = UDim2.new(0, 10, 0, y),
            Size = UDim2.new(0, Width, 0, BTN_H),
            Parent = MobileGui,
        })
