--[[
    Custom Library (flat/minimal design, based on Linoria structure)
    - Плоский дизайн виджетов (без толстых чёрных рамок, градиентная заливка) - как в приложенном C++/ImGui примере
    - Кастомный шрифт из base64 (.ttf), встраивается через getcustomasset
    - Детект исполнителя: если содержит "delta" -> компактное мобильное меню
    - Мобильные кнопки: Toggle UI / Lock UI (Unlock UI)
    - Лёгкая ватермарка и кейбинды: просто полоска сверху
    - Notify (message log): полоска сверху, выезжает как ватермарка
]]
...
------------------------------------------------------------
-- CUSTOM FONT (base64) --------------------------------------------------
-- Вставь сюда base64 своего .ttf (например ProggyClean.ttf).
-- Файлы будут сохранены рядом со скриптом: proggyclean.ttf / proggyclean.font
-- Если строка пустая - используется стандартный Enum.Font.Code.
------------------------------------------------------------
local b64decode = (crypt and crypt.base64 and crypt.base64.decode) or (crypt and crypt.base64decode) or base64_decode
local base64FontData = "" -- <-- ВСТАВЬ BASE64 .TTF СЮДА
...
------------------------------------------------------------
-- EXECUTOR DETECTION -> compact mobile menu -----------------------------
------------------------------------------------------------
...
-- Flat fill helper: vertical gradient rect, no thick border (matches C++ style)
...
------------------------------------------------------------
-- KEYBINDS (lightweight top-bar list) -------------------------------------
------------------------------------------------------------
do
    local Bar = Library:Create('Frame', {
        Name = 'Keybinds',
...
------------------------------------------------------------
-- NOTIFY / MESSAGE LOG (same light bar style, slides in from top) ---------
------------------------------------------------------------
do
...
function Funcs:AddToggle(Idx, Info)
    ...
    -- flat checkbox (vertical gradient fill square, thin single-color border only)
    ...
    Toggle.TextLabel = Row -- позволяет вешать AddKeyPicker/AddColorPicker в тот же ряд справа
...
function Library:CreateWindow(Config)
    ...
    function Window:AddTab(Name)
        ...
        local Content = Library:Create('Frame', { BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, 0), AutomaticSize = Enum.AutomaticSize.Y, Visible = false, Parent = Body })
        Library:Create('UIListLayout', { Padding = UDim.new(0, 8), SortOrder = Enum.SortOrder.LayoutOrder, Parent = Content })
        ...
        function Tab:AddGroupbox(Name)
            ...
            local BoxOuter = Library:Create('Frame', { BackgroundColor3 = Color3.fromRGB(20, 20, 20), BorderSizePixel = 0, Size = UDim2.new(1, 0, 0, 40), Parent = Content })
        ...
        if not next(Window.Tabs, next(Window.Tabs)) then Tab:Show() end -- показать первую вкладку
        return Tab
    end
...
------------------------------------------------------------
-- MOBILE MOD MENU (Delta detection / touch) --------------------------------
-- Компактные кнопки: Toggle UI / Lock UI <-> Unlock UI, под RB, слева сверху
------------------------------------------------------------
if IsMobileMode then
    ...
    local StartY = 78 -- чуть ниже RB-кнопки джойстика
    ...
    -- если компактный режим (Delta) - меньше окно и кнопки чуть плотнее уже учтено через BaseSize выше
end
